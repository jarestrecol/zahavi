...
export class Ingredient {
  ...
  cambiarCosto(
    ...
    if (nuevoCosto.toCop() < 0)
      return err(new CostoUnitarioNegativoError(nuevoCosto.toCop()));
  ...
  desactivar(ahora: FechaHora, eventoId: string): Result<Ingredient, IngredienteYaEnEstadoError> {
    if (this.estado === 'inactivo')
      return err(new IngredienteYaEnEstadoError('inactivo'));
  ...
  reactivar(ahora: FechaHora, eventoId: string): Result<Ingredient, IngredienteYaEnEstadoError> {
    if (this.estado === 'activo')
      return err(new IngredienteYaEnEstadoError('activo'));
