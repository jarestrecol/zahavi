...
export class Recipe {
  ...
  actualizarLinea(
    ...
    if (!existe)
      return err(new LineaDeRecetaNoEncontradaError(lineaActualizada.id.toString()));
    ...
    if (otrasLineas.some((l) => l.ingredientId.equals(lineaActualizada.ingredientId)))
      return err(
        new IngredienteDuplicadoEnRecetaError(lineaActualizada.ingredientId.toString()),
      );
  ...
  > {
    ...
    for (const linea of this.lineas) {
      if (!costos.has(linea.ingredientId.toString()))
        faltantes.push(linea.ingredientId.toString());
    }
