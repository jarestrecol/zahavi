...
export class ProductVariant {
  ...
  conPrecio(nuevoPrecio: Money): Result<ProductVariant, PrecioInvalidoError> {
    ...
    return ok(
      new ProductVariant({ id: this.id, nombre: this.nombre, precio: nuevoPrecio, recetaId: this.recetaId }),
    );
  }

  conNombre(nuevoNombre: NombreDeCatalogo): ProductVariant {
    return new ProductVariant({ id: this.id, nombre: nuevoNombre, precio: this.precio, recetaId: this.recetaId });
  }
