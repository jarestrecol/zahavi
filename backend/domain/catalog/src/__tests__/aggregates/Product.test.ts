...
describe('Product — vincularRecetaAVariante', () => {
  it('vincula receta a una variante existente', () => {
    ...
    expect(r.ok).toBe(true);
    if (r.ok) {
      const varianteActualizada = r.value.variantes.find((v) =>
        v.id.equals(unwrap(ProductVariantId.of(VARIANT_ID_1))),
      );
...
describe('Product — calcularMargen', () => {
  ...
  it('calcula margen positivo correctamente', () => {
    ...
    const r = producto.calcularMargen(unwrap(ProductVariantId.of(VARIANT_ID_1)), receta, costos);
    expect(r.ok).toBe(true);
    if (r.ok) {
  ...
  it('falla si la variante no existe en el producto', () => {
    ...
    const r = producto.calcularMargen(unwrap(ProductVariantId.of(VARIANT_ID_2)), receta, costos);
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error).toBeInstanceOf(VarianteNoEncontradaError);
  });
});
...
describe('Product — imagenUrl', () => {
  ...
  it('cambiarImagen actualiza la imagen y emite evento', () => {
    ...
    if (r.ok) {
      ...
      expect(r.value.pullDomainEvents().some((e) => e.tipo === 'ImagenDeProductoCambiada')).toBe(
        true,
      );
    }
  });
